...
class CatService extends ChangeNotifier {
  ...
  CatService(this.prefs) {
    getRandomCatImage();
    favoriteImage = prefs.getStringList('favorites') ?? [];
  }
  ...
  void toggleFavoriteImage(String catImage) {
    if (favoriteImage.contains(catImage)) {
      favoriteImage.remove(catImage);
    } else {
      favoriteImage.add(catImage);
    }
    prefs.setStringList('favorites', favoriteImage);
    notifyListeners();
  }
}
                ...
                return GestureDetector(
                  ...
                  child: Stack(children: [
                    ...
                    Positioned(
                        ...
                        child: Icon(
                          Icons.favorite,
                          color: catService.favoriteImage.contains(catImage)
                              ? Colors.amber
                              : Colors.transparent,
                        ))
                  ]),
      ...
      builder: (context, catService, child) {
        return Scaffold(
          ...
          body: GridView.count(
            ...
            crossAxisCount: 2,
            children: List.generate(
              catService.favoriteImage.length,
              (index) {
                String catImage = catService.favoriteImage[index];
                return GestureDetector(
                  onTap: () {
                    catService.toggleFavoriteImage(catImage);
                  },
